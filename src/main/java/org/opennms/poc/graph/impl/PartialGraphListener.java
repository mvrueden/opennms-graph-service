...
public class PartialGraphListener implements GraphListener {
    ...
    public PartialGraphListener(String namespace, GraphRepository graphRepository) {
    ...
    public void handleGraphChangeStartEvent(GraphChangeStartedEvent startedEvent) {
        if (startedEvent.getNamespace().equalsIgnoreCase(namespace)) {
            LOG.info("Graph Creation started");
            graph = new GenericGraph();
            graph.setNamespace(startedEvent.getNamespace());
        }
    }

    @Override
    public void handleGraphChangeEndEvent(GraphChangedFinishedEvent finishedEvent) {
        if (finishedEvent.getNamespace().equalsIgnoreCase(namespace)) {
